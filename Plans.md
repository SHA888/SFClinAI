...
| Task | Content | DoD | Depends | Status |
|------|---------|-----|---------|--------|
| 10.1 | Define `LlmProposerConfig` struct | Configuration struct holding: LLM endpoint (OpenAI / Anthropic / local), model name, prompt template, max tokens, temperature, seed for reproducibility. Supports offline mock mode (returns fixed canned responses) for CI/testing. | 8.1 | cc:wip |
