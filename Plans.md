...
|------|---------|-----|---------|--------|
| 13.1 | Define `Cap` / `CapacityHypothesis` type and `⊑_IS` poset, `⊤_IS` top element | Per DEF-IS-01/02: `CapacityHypothesis` struct with a partial-order `⊑_IS` ("at least as committed as"); `Cap::top()` constructs `⊤_IS` (fully uncommitted) satisfying `∀c. c ⊑_IS ⊤_IS`. Doc anchors to SPEC.md §3.1. Partial-meet poset structure (MC-1) documented. | 7.4 [tdd:required] | cc:done [bf976c9] |
