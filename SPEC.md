...
**OQ-PS-04 — INV-PS-01 (Compatibility under refinement) does not hold in v0.2.0-alpha.0.** The invariant states: if `h₁ ⊑_PS h₂` and `compat_PS(h₂, h₃)`, then `compat_PS(h₁, h₃)`. The current implementation of `compat_PS(h, h') = (h == h') ∨ h.is_unknown() ∨ h'.is_unknown()` is too strict to satisfy this under the refined `PartialOrd` (atom-set inclusion). Counterexample: h₁ = {a, b}, h₂ = Unknown, h₃ = {a, c}. Then h₁ ⊑_PS h₂ (true) and compat_PS(h₂, h₃) (true: Unknown is compatible with all), but ¬compat_PS(h₁, h₃) (false: {a,b} ≠ {a,c} and neither is Unknown). _Disposition:_ Deferred to v1.x. The current strict definition preserves semantic clarity ("compatible" means "equal or one is Unknown"). Phase 4 (operator-set formalization) does not depend on INV-PS-01; it depends on operator soundness (OBL-PS-03), which is orthogonal. Future revision with full ontology lattice structure (SNOMED hierarchies, contraindication rules) will enable proper compatibility checking; premature widening of `compat` now risks breaking Galois adjunction (OBL-PS-02) or operator composition properties. The gap is named and documented rather than papered over.
